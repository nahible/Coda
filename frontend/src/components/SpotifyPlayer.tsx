...
export default function SpotifyPlayer() {
  ...
  useEffect(() => {
    if (playing) {
      intervalRef.current = setInterval(() => {
        setElapsed((prev) => {
          if (prev >= track.duration - 1) { skipNext(); return 0; }
          return prev + 1;
        });
      }, 1000);
  ...
  function skipNext() { setTrackIdx((i) => (i + 1) % mockPlaylist.length); setElapsed(0); }
  function skipPrev() { elapsed > 3 ? setElapsed(0) : (setTrackIdx((i) => (i - 1 + mockPlaylist.length) % mockPlaylist.length), setElapsed(0)); }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 pb-5">
        <div
          ...
          onClick={(e) => {
            ...
            setElapsed(Math.floor(((e.clientX - rect.left) / rect.width) * track.duration));
          }}
        >
          <div className="h-full bg-accent-strong rounded-full transition-[width] duration-1000" style={{ width: `${pct}%` }} />
        </div>
      ...
      <div className="flex items-center justify-center gap-8">
        <button className="w-10 h-10 flex items-center justify-center rounded-full text-ink-muted hover:text-ink hover:bg-panel-inner transition-all duration-200 cursor-pointer" onClick={skipPrev} id="spotify-prev" title="Previous">
          <SkipBack size={17} strokeWidth={1.4} />
        </button>
        <button className="w-12 h-12 flex items-center justify-center rounded-full bg-accent-strong text-ink-on-accent shadow-[0_3px_12px_rgba(120,100,160,0.18)] hover:shadow-[0_5px_18px_rgba(120,100,160,0.25)] transition-all duration-200 cursor-pointer" onClick={() => setPlaying(!playing)} id="spotify-play" title={playing ? 'Pause' : 'Play'}>
          {playing ? <Pause size={18} strokeWidth={1.4} /> : <Play size={18} strokeWidth={1.4} />}
        </button>
        <button className="w-10 h-10 flex items-center justify-center rounded-full text-ink-muted hover:text-ink hover:bg-panel-inner transition-all duration-200 cursor-pointer" onClick={skipNext} id="spotify-next" title="Next">
