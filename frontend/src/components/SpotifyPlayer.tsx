...
export default function SpotifyPlayer() {
  ...
  useEffect(() => {
    fetchSpotifyStatus().then(setIsConnected).catch(() => setIsConnected(false));
  }, []);
  ...
  const handleSkipNext = async () => {
    ...
    setTimeout(() => { fetchNowPlaying().then(setTrack); }, 500); // Update after skip
  };

  const handleSkipPrev = async () => {
    await skipPrevSpotify();
    setTimeout(() => { fetchNowPlaying().then(setTrack); }, 500);
  };
